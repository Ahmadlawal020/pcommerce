  ...
  return (
    <div className="bg-white">
      <div className="w-screen max-w-screen-2xl  overflow-hidden flex items-center justify-between h-20 ml-auto mr-auto max-sm:px-2 max-md:px-4 max-lg:px-8 max-xl:px-16 px-23 max-sm:h-14">
        ...
        <div className="max-md:hidden flex-grow">
          <SearchInput />
        </div>

        <div className="flex items-center gap-4 mx-3">
          <Link className="hidden sm:inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-700 hover:text-green-600 transition-colors border border-gray-200 rounded-md hover:border-green-300 whitespace-nowrap cursor-pointer">
            Check Order Status
          </Link>
          <Link
            to={"/auth"}
      ...
      <div>
        <SearchInput searchBtnStyle="max-w-screen-2xl ml-auto mr-auto max-sm:px-2 max-md:px-4 max-lg:px-8 max-xl:px-16 px-36 md:hidden py-2" />
      </div>
    </div>
  );
};
